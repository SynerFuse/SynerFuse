        ...
        if parallel_state.get_virtual_pipeline_model_parallel_world_size() is not None:
            ...
            if parallel_state.get_pipeline_model_parallel_world_size() > 1:
                if self.config.enable_hetero:
                    if self.config.hetero_pipeline_stages is not None and self.config.hetero_pipeline_layer_split is not None:
                        raise ValueError("Cannot set both hetero_pipeline_stages and hetero_pipeline_layer_split simultaneously.")
                    elif self.config.hetero_pipeline_layer_split is not None:
                        offset = sum(([0] + self.config.hetero_pipeline_layer_split)[: pipeline_rank + 1])
                    elif self.config.hetero_pipeline_stages is not None:
